...
def main(args):
	...
	for language_entry in languages:
		...
		for static_entry in doc_json['statics']:
			static = static_entry['source']
			file = os.path.join(template_dir,'source',language_dir,static)
			if not os.path.exists(file):
				file2 = os.path.join(template_dir,'source',static)
				if os.path.exists(file2):
					...
					continue
			generate_template(file,static_entry,language,language_code,language_dir,languages,sections)
